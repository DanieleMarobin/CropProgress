...
def extract_GE_conditions(df):
    ...
    # df=df.drop_duplicates(subset=['year','reference_period_desc','short_desc'], keep='last')
    ...
    # the below 2 are to correct USDA wrong data
    mask=(df['week_ending'].dt.month>9) & (df['week_ending'].dt.year>=df['year'])
...
def get_USA_yields(commodity='CORN', aggregate_level='NATIONAL', state_name=[], years=[], cols_subset=[]):
    ...
    return fo
def get_USA_yields_parallel(commodity='CORN', aggregate_level='STATE', state_name=[], years=[], cols_subset=[]):
    dfs={}

    with concurrent.futures.ThreadPoolExecutor(max_workers=100) as executor:
        results={}
        for s in state_name:
            results[s] = executor.submit(get_USA_yields, commodity, aggregate_level, [s], years, cols_subset)
...
def get_USA_prod_weights(commodity='CORN', aggregate_level='STATE', years=[], subset=[], pivot_column='state_name'):
    # pivot_column= 'state_name', 'state_alpha'
    # rows:       years
    # columns:    region
    
    fo=get_USA_production(commodity=commodity,aggregate_level=aggregate_level, years=years)
...
def get_USA_production(commodity='CORN', aggregate_level='NATIONAL', state_name=[], years=[], cols_subset=[]):
    ...
    return fo
def get_USA_production_parallel(commodity='CORN', aggregate_level='STATE', state_name=[], years=[], cols_subset=[]):
    dfs={}

    with concurrent.futures.ThreadPoolExecutor(max_workers=100) as executor:
        results={}
        for s in state_name:
            results[s] = executor.submit(get_USA_production, commodity, aggregate_level, [s], years, cols_subset)
...
def get_USA_acreage(commodity='CORN', aggregate_level='NATIONAL', state_name=[], years=[], cols_subset=[]):
    """
    df_prod=qs.get_QS_production('soybeans', aggregate_level='STATE', years=[2017])\n
    ...
    dl.short_desc=[commodity+' - ACRES PLANTED', commodity+' - ACRES HARVESTED']
